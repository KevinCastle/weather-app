...
const ForecastAPI = (props) => {
    ...
    useEffect(() => {
        axios.get(`http://api.openweathermap.org/data/2.5/forecast?lat=${props.latitude}&lon=${props.longitude}&units=metric&appid=${props.appid}`)
            .then(res => {
                const data = res.data;
                setDays(data.list);
                setIsFetched(true);
            ...
            <ul>
                {isFetched && ListDay.map((day) =>
                    <li key={day[0].dt_txt.substring(0, 10)}>
                        <Link to={`/${day[0].dt_txt.substring(0, 10)}`}>{day[0].dt_txt.substring(0, 10)}</Link>
                    </li>
                )}
            </ul>
            <Switch>
